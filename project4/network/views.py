...
def save_post(request):
    if request.method == "POST":
        ...
        profileuser = Profile(user = request.user)
        # it checks if Profile model exist request.user to provide first register user can post     
        if not Profile.objects.filter(user=request.user).exists():
            print("user does not exist in Profile model")
            profileuser.save()
            print("saved")
...
# follow vs unfollow toggle add/remove
@login_required 
